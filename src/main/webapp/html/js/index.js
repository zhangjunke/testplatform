...
var urlperfix="/dubbomock";
var searchUrl=urlperfix+'/mocker/MockDetailSearch';
    ...
    $.post(searchUrl, data, function (list1) {
        ...
        if (list != null) { //后台传回来的select选项
            ...
            for (var i = 0; i < list.length; i++) {
               ...
               var mockCondition1=list[i].mockCondition;
               var mockCondition=mockCondition1.substring(0,mockCondition1.length-1);
               ...
               conditionList.set(JSON.stringify(i),JSON.stringify(mockCondition));
                ...
                mockMsg=document.createElement('td');
                ...
                mockMsg.innerHTML=list[i].mockResponseMsg;
                mock_timeout.innerHTML=list[i].mock_timeout;
                mockCondition.innerHTML=mockCondition1.substring(0,mockCondition1.length-1);
...
function modifyDetail(i,id){
    ...
    window.open("edit_mocker.html?"+"mockCaseName="+encodeURI(caseList.get(i))+"&amp;mockTime="+encodeURI(timeList.get(i))+
        "&amp;mockMsg="+encodeURI(msgList.get(i))+"&amp;mockCondition="+encodeURI(conditionList.get(i))+"&amp;id="+encodeURI(id)+"&amp;API="+encodeURI(API));
}
...
function conditionSetting(detailid){
    var url=urlperfix+"/mocker/MockConditionSearch";
    var data = {'mockDetail_id': detailid};
    ...
    $.post(url, data, function (string) {
        ...
        window.open(urlperfix+"conditionSetting.html?"+S);
        });
}
function warning(){
    alert("暂不提供该功能！");
